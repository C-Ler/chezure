...
(check (chezure-captures-names re-named) => '("year" "month" "day"))
(let ([caps (car (chezure-find-captures re-named "1992-01-03, 1993-10-01, 1992-04-10" 1))])
  (check (chezure-captures-names caps) => '("year" "month" "day")))

(let ([caps (car (chezure-find-captures re-named "1992-01-03, 1993-10-01, 1992-04-10" 1))])
  (check (chezure-captures-string-ref caps 0) => "1992-01-03")
  (check (chezure-captures-string-ref caps 1) => "1992")
  (check (chezure-captures-string-ref caps 2) => "01")
  (check (chezure-captures-string-ref caps 3) => "03")
  (check (chezure-captures-string-ref caps "year") => "1992")
  (check (chezure-captures-string-ref caps "month") => "01")
  (check (chezure-captures-string-ref caps "day") => "03"))

(let ([caps (cadr (chezure-find-captures re-named "1992-01-03, 1993-10-01, 1992-04-10" 2))])
  (check (chezure-captures-string-ref caps 0) => "1993-10-01")
  (check (chezure-captures-string-ref caps 1) => "1993")
  (check (chezure-captures-string-ref caps 2) => "10")
  (check (chezure-captures-string-ref caps 3) => "01")
  (check (chezure-captures-string-ref caps "year") => "1993")
  (check (chezure-captures-string-ref caps "month") => "10")
  (check (chezure-captures-string-ref caps "day") => "01"))

(let ([caps (caddr (chezure-find-captures re-named "1992-01-03, 1993-10-01, 1992-04-10" 10))])
  (check (chezure-captures-string-ref caps 0) => "1992-04-10")
  (check (chezure-captures-string-ref caps 1) => "1992")
  (check (chezure-captures-string-ref caps 2) => "04")
  (check (chezure-captures-string-ref caps 3) => "10")
  (check (chezure-captures-string-ref caps "year") => "1992")
  (check (chezure-captures-string-ref caps "month") => "04")
  (check (chezure-captures-string-ref caps "day") => "10"))

(let ([caps (car (chezure-find-captures re-named "1992-01-03, 1993-10-01, 1992-04-10" 1))])
  (check (chezure-captures-string-ref caps '(1 2)) => '("1992" "01"))
  (check (chezure-captures-string-ref caps (vector 1 3)) => (vector "1992" "03"))
  (check (chezure-captures-string-ref caps (vector "day" "year")) => (vector "03" "1992")))

(check (chezure-captures-names re-named-unicode) => '("country" "continent"))
(let ([caps (car (chezure-find-captures re-named-unicode "中国是亚洲国家，瑞典是欧洲国家"))])
  (check (chezure-captures-string-ref caps 0) => "中国是亚洲国家")
  (check (chezure-captures-string-ref caps 1) => "中国")
  (check (chezure-captures-string-ref caps 2) => "亚洲")
  (check (chezure-captures-string-ref caps "country") => "中国")
  (check (chezure-captures-string-ref caps "continent") => "亚洲"))

(let ([caps (cadr (chezure-find-captures re-named-unicode "中国是亚洲国家，瑞典是欧洲国家"))])
  (check (chezure-captures-string-ref caps 0) => "瑞典是欧洲国家")
  (check (chezure-captures-string-ref caps 1) => "瑞典")
  (check (chezure-captures-string-ref caps 2) => "欧洲")
  (check (chezure-captures-string-ref caps "country") => "瑞典")
  (check (chezure-captures-string-ref caps "continent") => "欧洲"))
...
(let ([repl (lambda (caps)
              (if (string=? (chezure-captures-string-ref caps "year")
                            "1992")
                  "the year of 1992"
                  (chezure-captures-string-ref caps 0)))])
...
(check (chezure-replace re-unicode "中国与香港的关系走向何方？" "<COUNTRY>" 1)
       => "<COUNTRY>与香港的关系走向何方？")
(let ([repl (lambda (caps)
              (if (and (string=? (chezure-captures-string-ref caps "country") "中国")
                       (string=? (chezure-captures-string-ref caps "continent") "亚洲"))
                  "日本是亚洲国家"
                  (chezure-captures-string-ref caps 0)))])
